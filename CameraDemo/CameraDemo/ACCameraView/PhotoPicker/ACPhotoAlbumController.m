...
- (UITableView *)albumTableView {
    if (!_albumTableView) {
        ...
        _albumTableView.contentInset = UIEdgeInsetsMake(ACCAMERA_NAVI_HEIGHT, 0, 15.0f, 0);
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ACPhotoAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ACPhotoAlbumCell class]) forIndexPath:indexPath];
        ...
        if (fetchResult.count > 0) {
            ...
            [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:CGSizeMake(100*ACCAMERA_SCREEN_SCALE, 100*ACCAMERA_SCREEN_SCALE) contentMode:PHImageContentModeAspectFit options:_requestOption resultHandler:^(UIImage *result, NSDictionary *info) {
