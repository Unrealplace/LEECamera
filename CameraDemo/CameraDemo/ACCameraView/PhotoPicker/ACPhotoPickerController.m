...
- (void)loadView {
    [super loadView];
    self.view.backgroundColor = ACCAMERA_ACRGBColor(51, 51, 51);
...
- (UIView *)naviView {
    if (!_naviView) {
        _naviView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.ca_width, ACCAMERA_NAVI_HEIGHT + ACCAMERA_NAVI_TOP_PADDING)];
        ...
        naviBgView.backgroundColor = ACCAMERA_RGBAColor(51, 51, 51, 0.95);
        [_naviView addSubview:naviBgView];
        
        CGFloat btnSize = ACCAMERA_DeviceIsX ? 44.0f : _naviView.ca_height;
        UIButton * closeBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, ACCAMERA_NAVI_TOP_PADDING + (ACCAMERA_NAVI_HEIGHT - btnSize) / 2, btnSize, btnSize)];
...
- (UICollectionView *)photoCollections
{
    if (_photoCollections == nil)
    {
        ...
        layout.sectionInset                 = UIEdgeInsetsMake(ACCAMERA_NAVI_HEIGHT, 0, 0, 0);
